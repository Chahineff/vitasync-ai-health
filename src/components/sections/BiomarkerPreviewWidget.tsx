...
const statusColors = { optimal: "text-emerald-500", low: "text-amber-500", high: "text-red-400" };
const statusBg = { optimal: "bg-emerald-500", low: "bg-amber-500", high: "bg-red-400" };
...
export function BiomarkerPreviewWidget() {
  ...
  useEffect(() => {
    ...
    if (phase === "dropzone") timer = setTimeout(() => setPhase("dragging"), 1200);
    else if (phase === "dragging") timer = setTimeout(() => setPhase("dropped"), 1400);
    else if (phase === "dropped") timer = setTimeout(() => setPhase("scanning"), 600);
  ...
  useEffect(() => {
    ...
    if (visibleLines >= biomarkers.length + 1) {
      const timer = setTimeout(() => { setPhase("dropzone"); setVisibleLines(0); setScanProgress(0); }, 4000);
      return () => clearTimeout(timer);
    }
  ...
  return (
    <div className="w-full h-full flex flex-col gap-1.5 sm:gap-2.5 p-0.5 sm:p-1">
      <AnimatePresence mode="wait">
        {showDropzone && (
          <motion.div key="dropzone" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.3 }}
            ...
            {phase === "dragging" && (
              <motion.div className="absolute inset-0 bg-primary/5 rounded-2xl" animate={{ opacity: [0.3, 0.6, 0.3] }} transition={{ duration: 1, repeat: Infinity }} />
            )}
            <motion.div animate={phase === "dragging" ? { scale: [1, 1.1, 1] } : {}} transition={{ duration: 0.8, repeat: Infinity }}
            ...
            <span className="text-[10px] text-foreground/40 font-medium">
              {phase === "dropzone" ? t("preview.biomarker.dropHere") : phase === "dragging" ? t("preview.biomarker.dropFile") : ""}
            </span>
            {(phase === "dragging" || phase === "dropped") && (
              <motion.div className="absolute"
                initial={{ x: 80, y: -60, rotate: -8, scale: 0.7, opacity: 0 }}
                animate={phase === "dropped" ? { x: 0, y: 0, rotate: 0, scale: 1, opacity: 1 } : { x: 30, y: -20, rotate: -5, scale: 0.85, opacity: 1 }}
                transition={phase === "dropped" ? { type: "spring", stiffness: 300, damping: 20 } : { duration: 0.6, ease: "easeOut" }}>
            ...
            {phase === "dropped" && (
              <motion.div className="absolute inset-0 rounded-2xl border-2 border-primary/40"
                initial={{ opacity: 1, scale: 0.95 }} animate={{ opacity: 0, scale: 1.05 }} transition={{ duration: 0.6 }} />
            )}
          </motion.div>
        )}
          ...
          <motion.div key="scanning" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            ...
            <div className="relative w-14 h-18">
              <div className="w-14 h-[72px] rounded-lg bg-card border border-border/60 flex flex-col items-center justify-center gap-1 overflow-hidden relative">
                ...
                <motion.div className="absolute left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-primary to-transparent"
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-1 flex items-center gap-1.5">
                          <div className="flex-1 h-1 bg-muted/60 rounded-full relative overflow-hidden">
                            <motion.div className={`absolute inset-y-0 left-0 rounded-full ${statusBg[bm.status]}`}
                              initial={{ width: 0 }} animate={{ width: `${bm.bar}%` }} transition={{ duration: 0.6, delay: 0.1 }} />
                          </div>
