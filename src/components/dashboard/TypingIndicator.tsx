...
export const TypingIndicator = ({ selectedModel }: TypingIndicatorProps) => {
  ...
  const THINKING_PHRASES = [
    t("typing.analyzing"),
    t("typing.consultingData"),
    t("typing.preparingResponse"),
    t("typing.thinking"),
  ];
      ...
      <div className="relative w-10 h-10 flex-shrink-0">
        ...
        <motion.div 
          ...
          animate={isSimpleMode ? {} : { 
            scale: [0.9, 1.1, 0.9],
            boxShadow: ["0 0 10px 2px rgba(0, 240, 255, 0.15)", "0 0 25px 6px rgba(0, 240, 255, 0.4)", "0 0 10px 2px rgba(0, 240, 255, 0.15)"]
          }}
      ...
      <div className="flex-1 pt-2">
        ...
        {isSimpleMode ? (
          <div className="flex items-center gap-1.5 mb-4">
            {[0, 1, 2].map((i) => (
              <motion.div key={i} className="w-2 h-2 rounded-full bg-primary/60" animate={{ opacity: [0.3, 1, 0.3], scale: [0.8, 1.1, 0.8] }} transition={{ duration: 1, repeat: Infinity, delay: i * 0.2 }} />
            ))}
          </div>
        ) : (
          <>
            ...
            <div className="relative h-0.5 bg-muted dark:bg-white/10 rounded-full overflow-hidden">
              <motion.div className="absolute top-0 h-full w-8 rounded-full bg-gradient-to-r from-transparent via-primary to-transparent" animate={{ left: ['-10%', '110%'] }} transition={{ duration: 1.5, repeat: Infinity, ease: "easeInOut" }} />
            </div>
          </>
        )}
      </div>
